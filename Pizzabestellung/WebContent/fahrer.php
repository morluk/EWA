...
class Fahrer extends Page
{
    ...
    protected function getViewData()
    {
        ...
        $sql = "SELECT Bestellung.BestellungId, Bestellung.Adresse, SUM(Angebot.Preis) AS Gesamtpreis
        		...
        		Angebot.PizzaName = BestelltePizza.fPizzaName GROUP BY Bestellung.BestellungId";
    ...
    private function insert_article($bestellung) {
    	...
    	$pizzaNamen = $this->pizzaNamen[$id];
    	$preis = $bestellung["Gesamtpreis"];
	...
	<article class="fahrer">
		...
		<form id="formid" action="http://localhost$uri"
			...
			<table>
				...
				<tr>
					...
					<td class="status"><input type="radio" name=$id value="2"
						onclick="document.forms['formid'].submit();" $status_1 /></td>
					<td class="status"><input type="radio" name=$id value="3"
						onclick="document.forms['formid'].submit();" $status_2 /></td>
					<td class="status"><input type="radio" name=$id value="4"
						onclick="document.forms['formid'].submit();" $status_3 /></td>
...
HERE;
    ...
    {
        $this->getViewData ();
        $this->generatePageHeader("Pizzaservice", "Fahrer");
    ...
    protected function processReceivedData() 
    {
        ...
        $sql = "SELECT fBestellungId from BestelltePizza GROUP BY fBestellungId";
        $recordset = $this->_database->query ( $sql );
        ...
        foreach ( $bestellungId as $id ) {
        	if (isset ( $_POST [$id ["fBestellungId"]] )) {
        		$escapedStatus = $this->_database->real_escape_string ( $_POST [$id ["fBestellungId"]] );
        		$sqlUpdate = "UPDATE `BestelltePizza` SET `Status`=" . $escapedStatus . " WHERE `fBestellungId`=" . $id["fBestellungId"];
